...
E = P / 'evidence'
INPUT_HEAD = 'f8d4d5ee6f7509d1f5e9b6522f774e5e97ea119e'
BATCH_START, BATCH_END = 1501, 1560
...
research={}
with (E/'W3_BATCH_0026_RESEARCH.tsv').open('r',encoding='utf-8-sig',newline='') as f:
...
out=E/'W3_BATCH_0026_RESULTS.csv'
...
for path in [ledger_path,overlay_path]:
    ...
    assert len(z)==1560 and [int(x['queue_order']) for x in z]==list(range(1,1561)) and len({x['queue_order'] for x in z})==1560

metrics=f'''schema: fc-exec-2515-w3-batch-metrics-v1\nproject_id: FC-EXEC-2515-01\ngate: W3-BATCH-0026\nstatus: SEALED\nvalidation: PASS\ntouched_rows: 60\nfujian_rows: 60\np1_identity_rows: 60\nevidence_backed_rows: 60\na_grade_rows: {grades['A']}\nb_grade_rows: {grades['B']}\nc_grade_rows: {grades['C']}\ncurrent_org_title_confirmed_rows: {confirmed}\nno_current_role_confirmed_rows: {no_role}\ncurrent_org_title_unresolved_rows: {unresolved}\nunsupported_current_title_claims: 0\nqueue_start: 1501\nqueue_end: 1560\nnext_queue_order: 1561\nverified_at: {NOW}\ninput_head: {INPUT_HEAD}\n'''
(E/'W3_BATCH_0026_METRICS.yaml').write_text(metrics,encoding='utf-8')
validation=f'''# W3-BATCH-0026 Validation\n\n- Gate: `W3-BATCH-0026`\n- Queue orders: `1501-1560`\n- Touched rows: **60** (`福建 60`)\n- Priority: `P1_IDENTITY` 60\n- Evidence coverage: **60/60**\n- Evidence grades: **A={grades['A']}, B={grades['B']}, C={grades['C']}**\n- Current organization + title confirmed: **{confirmed}**\n- No-current-role confirmed: **{no_role}**\n- Explicit current-role unresolved: **{unresolved}**\n- Unsupported current-title claims: **0**\n- Cumulative evidence ledger: **1560 rows**, queue `1-1560` contiguous, no duplicate queue order\n- Cumulative verification overlay: **1560 rows**, queue `1-1560` contiguous, no duplicate queue order\n\n## Identity / recency controls\n\n- 58 cross-listed 福建 rows reused sealed evidence only after exact repository `ambiguity_group_id + province + normalized name + region qualifier` match; every source row retains an independent `person_id`.\n- The only two fresh rows are two distinct source records both named 林飞 but explicitly qualified by the source as `福州` and `龙岩`; they were researched separately and were never merged by name.\n- `林飞(龙岩)` is safely closed to **龙岩市交通运输有限公司董事长**: a 2026 Dragon-rock youth/entrepreneur profile states the company title and 龙岩市青年企业家协会会长 role together, while a 2025 福建省人民政府 decision independently lists 林飞 as the same company chairman.\n- `林飞(福州)` remains unresolved because current public results contain several materially different 福州/福建 people named 林飞 and no authority source safely bridges one of them to the provincial federation-executive source record.\n\n**PASS**\n'''
(E/'W3_BATCH_0026_VALIDATION.md').write_text(validation,encoding='utf-8')

sp=P/'STATUS.yaml'; s=sp.read_text(encoding='utf-8')
repls={
'code: W3-BATCH-0025\n  phase: W3-EVIDENCE-ENRICH\n  status: SEALED':'code: W3-BATCH-0026\n  phase: W3-EVIDENCE-ENRICH\n  status: SEALED',
'code: W3-BATCH-0026\n  phase: W3-EVIDENCE-ENRICH\n  status: READY':'code: W3-BATCH-0027\n  phase: W3-EVIDENCE-ENRICH\n  status: READY',
'batches_sealed: 25':'batches_sealed: 26','processed_queue_rows: 1500':'processed_queue_rows: 1560','next_queue_order: 1501':'next_queue_order: 1561','actionable_queue_remaining: 9144':'actionable_queue_remaining: 9084','processed: 1399':'processed: 1459','remaining: 2932':'remaining: 2872','identity_evidence_rows: 1500':'identity_evidence_rows: 1560','current_org_title_confirmed_rows: 842':f'current_org_title_confirmed_rows: {842+confirmed}','current_org_title_unresolved_rows: 555':f'current_org_title_unresolved_rows: {555+unresolved}','no_current_role_confirmed_rows: 2':f'no_current_role_confirmed_rows: {2+no_role}'}
for a,b in repls.items(): assert a in s,a; s=s.replace(a,b,1)
anchor='  w3_batch_0025_validation: projects/fc-exec-2515/evidence/W3_BATCH_0025_VALIDATION.md\n'; assert anchor in s
s=s.replace(anchor,anchor+'  w3_batch_0026_research: projects/fc-exec-2515/evidence/W3_BATCH_0026_RESEARCH.tsv\n  w3_batch_0026_results: projects/fc-exec-2515/evidence/W3_BATCH_0026_RESULTS.csv\n  w3_batch_0026_metrics: projects/fc-exec-2515/evidence/W3_BATCH_0026_METRICS.yaml\n  w3_batch_0026_validation: projects/fc-exec-2515/evidence/W3_BATCH_0026_VALIDATION.md\n',1); sp.write_text(s,encoding='utf-8')

cp=P/'CURRENT.yaml'; c=cp.read_text(encoding='utf-8'); verified_add=confirmed+no_role
repls={'current_gate: W3-BATCH-0026':'current_gate: W3-BATCH-0027','work_cursor: 1500':'work_cursor: 1560','verified_records: 844':f'verified_records: {844+verified_add}','unresolved_records: 9800':f'unresolved_records: {9800-verified_add}','processed: 1500':'processed: 1560','actionable_remaining: 9144':'actionable_remaining: 9084','next_queue_order: 1501':'next_queue_order: 1561','processed: 1399':'processed: 1459','remaining: 2932':'remaining: 2872','last_input_head: f15d67eba53b4db0b502a47fba8435efafcce16d':f'last_input_head: {INPUT_HEAD}'}
...
c=c.rstrip()+f'''\n  - W3-BATCH-0026 processed queue orders 1501-1560 exactly, all 60 福建 P1 identity/current-role rows.\n  - {confirmed} rows received current organization/title confirmation; {unresolved} remain explicit unresolved; {no_role} have no-current-role confirmation; unsupported current-title claims remain zero.\n  - 58 rows reused prior sealed evidence only through exact repository ambiguity_group_id + province + normalized name + region qualifier match; independent person_id values are retained.\n  - The two fresh same-name records 林飞(福州) and 林飞(龙岩) were researched separately. 林飞(龙岩) safely closes to 龙岩市交通运输有限公司董事长 via 2026 local entrepreneur context plus 2025 福建省人民政府 corroboration; 林飞(福州) remains unresolved because multiple homonyms exist and no source identity bridge was found.\n  - Evidence grades are A={grades['A']}, B={grades['B']}, C={grades['C']}; next deterministic queue order is 1561.\n'''; cp.write_text(c,encoding='utf-8')
print('PASS W3-BATCH-0026',dict(statuses),dict(grades),'fresh',fresh,'next=1561')
