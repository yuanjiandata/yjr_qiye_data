...
GATE="W3-BATCH-0075"
START,END=4441,4500
EXPECTED_RESULTS_SHA="b6ead39544fd4e713f75b3ec3fe2bae64c96a0811e26599f1a18478ee0df2d74"
EXPECTED_RESEARCH_SHA="ed273307f672e88b9255edbcb566e4319e78498ffdfec67f35a8f78ec89fba0b"
DIRECT_CURRENT_BRIDGE={4442,4450,4475,4489}
...
def main():
    ...
    results=read_csv(ev/"W3_BATCH_0075_RESULTS.csv")
    ...
    assert status_counts==Counter({"CURRENT_ORG_TITLE_CONFIRMED":10,"NO_CURRENT_ROLE_CONFIRMED":1,"UNRESOLVED_CURRENT_ORG_TITLE":49})
    assert grade_counts==Counter({"A":53,"B":7})
    for r in results:
        ...
        if status=="CURRENT_ORG_TITLE_CONFIRMED":
            ...
            assert r["evidence_url"] and not r["unresolved_reason"]
        else:
            assert not r["current_organization"] and not r["current_title"]
            assert r["unresolved_reason"]

    assert sha256(ev/"W3_BATCH_0075_RESULTS.csv")==EXPECTED_RESULTS_SHA
    assert sha256(ev/"W3_BATCH_0075_RESEARCH.tsv.gz")==EXPECTED_RESEARCH_SHA
    with gzip.open(ev/"W3_BATCH_0075_RESEARCH.tsv.gz","rt",encoding="utf-8-sig",newline="") as f:
    ...
    for q in [4441,4442,4450,4451,4452,4454,4460,4474,4475,4480,4489]:
        assert rmap[q]["current_evidence_url"]
        if q not in DIRECT_CURRENT_BRIDGE:
            assert rmap[q]["identity_bridge_url"]

    idx=read_csv(ev/"W3_SEGMENT_INDEX.csv")
    new=[r for r in idx if r["ordinal"]=="48"]
    assert len(new)==2 and {r["kind"] for r in new}=={"ledger","overlay"}
    assert all(r["queue_start"]=="4441" and r["queue_end"]=="4500" and r["row_count"]=="60" and r["base_or_parent_gate"]==GATE for r in new)

    status=(root/"STATUS.yaml").read_text(encoding="utf-8")
    current=(root/"CURRENT.yaml").read_text(encoding="utf-8")
    assert "code: W3-BATCH-0075" in status and "code: W3-BATCH-0076" in status
    assert "current_gate: W3-BATCH-0076" in current and "work_cursor: 4500" in current
    assert "P2_ORG_TITLE_LOOKUP:\n      initial: 6045\n      processed: 68\n      remaining: 5977" in current
