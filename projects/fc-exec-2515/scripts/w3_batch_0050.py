#!/usr/bin/env python3
"""Deterministic validator for FC-EXEC-2515 W3-BATCH-0050."""
...
START,END=2941,3000
EXPECTED_RESULTS_SHA256="f28895ee57251483a89d1e1f91d47c1d5975bff7af1f77da5a1af1347e39830b"
EXPECTED_RESEARCH_SHA256="9b4319c5a4a09da74c70e1a8b388bbf24b936955e95e2211cdadef5e66bddd78"
CONFIRMED={2941: ('泸州市工商业联合会', '主席、总商会会长', 'A'), 2944: ('成都市工商业联合会', '党组书记', 'A'), 2947: ('四川家福来实业集团有限公司', '总经理', 'C'), 2948: ('四川省工商业联合会', '常委', 'C'), 2959: ('四川启阳汽车集团有限公司', '董事长', 'B'), 2965: ('四川省工商联财税金融服务中心', '主任', 'B'), 2974: ('星瑞集团', '董事长兼总裁', 'B'), 2978: ('成都彩虹电器集团股份有限公司', '党委书记、总经理', 'B'), 2981: ('通威集团', '董事局主席', 'A'), 2982: ('中国人民政治协商会议内江市委员会', '副主席', 'B'), 2989: ('四川省工商业联合会', '一级巡视员', 'B'), 2991: ('四川省工商业联合会', '党组成员、副主席', 'A'), 2994: ('川开电气有限公司', '董事长', 'B'), 2996: ('四川省工商业联合会', '副主席', 'B')}

def main():
    with gzip.open(E/"W2_UNRESOLVED_QUEUE.csv.gz","rt",encoding="utf-8-sig",newline="") as f:
        queue=list(csv.DictReader(f))[START-1:END]
    rb=(E/"W3_BATCH_0050_RESULTS.csv").read_bytes()
    qb=(E/"W3_BATCH_0050_RESEARCH.tsv").read_bytes()
    ...
    assert Counter(r["current_verification_status"] for r in results)==Counter({"UNRESOLVED_CURRENT_ORG_TITLE":46,"CURRENT_ORG_TITLE_CONFIRMED":14})
    assert Counter(r["evidence_grade"] for r in results)==Counter({"A":50,"B":8,"C":2})
    assert Counter(r["decision"] for r in research)==Counter({"UNRESOLVED":46,"CONFIRMED":14})
    assert sum(r["mode"]=="FRESH_WEB_RESEARCH" for r in research)==12
    assert sum(r["mode"]=="REUSE_SEALED_CONFIRMED" for r in research)==9
    assert sum(r["mode"]=="REUSE_SEALED_UNRESOLVED" for r in research)==39
    ...
    assert byq[2994]["person_id"]!=byq[2995]["person_id"]
    assert byq[2994]["current_verification_status"]=="CURRENT_ORG_TITLE_CONFIRMED"
    assert byq[2995]["current_verification_status"]=="UNRESOLVED_CURRENT_ORG_TITLE"
    assert byq[2998]["person_id"]!=byq[2999]["person_id"]
    assert byq[2998]["current_verification_status"]==byq[2999]["current_verification_status"]=="UNRESOLVED_CURRENT_ORG_TITLE"
    ...
    for kind in ("ledger","overlay"):
        ...
        assert expected_start==3001
        last=entries[-1]
        assert last["base_or_parent_gate"]=="W3-BATCH-0050"
        assert last["path"]=="projects/fc-exec-2515/evidence/W3_BATCH_0050_RESULTS.csv"
        assert int(last["queue_start"])==2941 and int(last["queue_end"])==3000
    print("PASS W3-BATCH-0050 rows=60 confirmed=14 unresolved=46 grades=A50/B8/C2 logical_end=3000")
