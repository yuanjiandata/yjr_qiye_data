#!/usr/bin/env python3
"""Deterministic validator for FC-EXEC-2515 W3-BATCH-0048."""
...
START,END=2821,2880
EXPECTED_RESULTS_SHA256="7069ed297fe5867f396626ae837779494b8bf51108e00010de6748f80fce8b03"
EXPECTED_RESEARCH_SHA256="18d427befc14fd75a77e2978d296419d444c1635296332ab4d4dc8f1e2ca4368"
CONFIRMED={2833: ('四川省工商联财税金融服务中心', '主任', 'B'), 2842: ('星瑞集团', '董事长兼总裁', 'B'), 2845: ('成都彩虹电器集团股份有限公司', '党委书记、总经理', 'B'), 2848: ('中国人民政治协商会议内江市委员会', '副主席', 'B'), 2859: ('四川英创力电子科技股份有限公司', '董事长', 'B'), 2866: ('自贡市工商业联合会', '主席、总商会会长', 'B'), 2868: ('中国人民政治协商会议广元市委员会', '副主席', 'B'), 2879: ('新希望集团', '党委书记、副董事长', 'B')}

def main():
    with gzip.open(E/"W2_UNRESOLVED_QUEUE.csv.gz","rt",encoding="utf-8-sig",newline="") as f:
        queue=list(csv.DictReader(f))[START-1:END]
    rb=(E/"W3_BATCH_0048_RESULTS.csv").read_bytes()
    qb=(E/"W3_BATCH_0048_RESEARCH.tsv").read_bytes()
    ...
    assert Counter(r["current_verification_status"] for r in results)==Counter({"UNRESOLVED_CURRENT_ORG_TITLE":52,"CURRENT_ORG_TITLE_CONFIRMED":8})
    assert Counter(r["evidence_grade"] for r in results)==Counter({"A":52,"B":8})
    assert Counter(r["decision"] for r in research)==Counter({"UNRESOLVED":52,"CONFIRMED":8})
    assert sum(r["mode"]=="FRESH_WEB_RESEARCH" for r in research)==53
    assert sum(r["mode"]=="REUSE_SEALED_CONFIRMED" for r in research)==1
    assert sum(r["mode"]=="REUSE_SEALED_UNRESOLVED" for r in research)==6
    ...
    for kind in ("ledger","overlay"):
        ...
        assert expected_start==2881
        last=entries[-1]
        assert last["base_or_parent_gate"]=="W3-BATCH-0048"
        assert last["path"]=="projects/fc-exec-2515/evidence/W3_BATCH_0048_RESULTS.csv"
        assert int(last["queue_start"])==2821 and int(last["queue_end"])==2880
    print("PASS W3-BATCH-0048 rows=60 confirmed=8 unresolved=52 grades=A52/B8/C0 logical_end=2880")
